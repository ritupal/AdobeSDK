...
#ifndef AEPIDENTITY_SWIFT_H
#define AEPIDENTITY_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="AEPIdentity",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
# pragma pop_macro("any")
#endif


@class NSString;
enum AEPMobileVisitorAuthState : NSInteger;

/// Defines  a visitor identifier
SWIFT_PROTOCOL_NAMED("Identifiable")
...
@property (nonatomic, readonly) enum AEPMobileVisitorAuthState authenticationState;
@end
...
@interface AEPMobileIdentity : NSObject <AEPExtension>
...
- (nonnull instancetype)initWithRuntime:(id <AEPExtensionRuntime> _Nonnull)runtime OBJC_DESIGNATED_INITIALIZER;
- (void)onRegistered;
- (void)onUnregistered;
...
@interface AEPMobileIdentity (SWIFT_EXTENSION(AEPIdentity))
/// Appends visitor information to the given URL.
/// \param url URL to which the visitor info needs to be appended. Returned as is if it is nil or empty.
///
/// \param completion closure which will be invoked once the updated url is available, along with an error if any occurred
///
...
/// \param completion closure which will be invoked once the customer identifiers are available.
///
...
/// \param completion closure which will be invoked once Experience Cloud ID is available.
///
...
///
/// \param authenticationState a <code>MobileVisitorAuthenticationState</code> value
///
...
/// Updates the given customer IDs with the Adobe Experience Cloud ID Service with authentication value of <code>MobileVisitorAuthenticationState.unknown</code>
/// \param identifiers a dictionary containing identifier type as the key and identifier as the value, both identifier type and identifier should be non empty and non nil values.
///
...
/// \param completion closure invoked with a value containing the visitor identifiers as a query string upon completion of the service request
///
...
typedef SWIFT_ENUM_NAMED(NSInteger, AEPMobileVisitorAuthState, "MobileVisitorAuthenticationState", open) {
  AEPMobileVisitorAuthStateUnknown = 0,
  AEPMobileVisitorAuthStateAuthenticated = 1,
  AEPMobileVisitorAuthStateLoggedOut = 2,
};
