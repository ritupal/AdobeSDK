...
#ifndef AEPANALYTICS_SWIFT_H
#define AEPANALYTICS_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="AEPAnalytics",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@interface AEPMobileAnalytics : NSObject <AEPExtension>
...
/// Invoked when the Analytics extension has been registered by the <code>EventHub</code>
- (void)onRegistered;
/// Invoked when the Analytics extension has been unregistered by the <code>EventHub</code>, currently a no-op.
- (void)onUnregistered;
/// Called before each <code>Event</code> processed by Analytics extension
/// \param event event that will be processed next
///
///
/// returns:
/// <em>true</em> if Configuration and Identity shared states are available
...
@interface AEPMobileAnalytics (SWIFT_EXTENSION(AEPAnalytics))
